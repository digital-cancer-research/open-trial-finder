...
# form query
openStudiesQ <- paste0("SELECT s.nct_id, s.brief_title, s.overall_status, c.name AS condition, i.name AS interventions, 
...
#if("lat" %in% names(cancerStudies) & "long" %in% names(cancerStudies)) {
## rename latitude and longitude columns
# use new_name = old_name syntax
cancerStudies <- dplyr::rename(cancerStudies, c("lat"="latitude" ,"long"="longitude"))
#}
...
```{r download a list of all human genes and their synonyms}
...
```{r disconnect from clinicaltrials}
dbDisconnect(conn2)
