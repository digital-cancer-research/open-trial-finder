...
# ## add a column that will hold matching gene names
# eligibilities$gene <- NA
# # create a variable that will hold matching substring
...
# mutationWordRange <- 6
# 
# ## loop through each criterion that contains a mutant pattern
# for(i in 1:nrow(eligibilities)) {
#   if(!is.na(eligibilities$change[i]) & eligibilities$change[i]=="MUTATION") {
...
#     indices <- grep(pattern = mutantPattern, x = criterion, ignore.case = TRUE)
#     ## specify start and end point(s) based on specified word range
#     starts <- indices - mutationWordRange
#     ends <- indices + mutationWordRange
#     range <- numeric(0)
#     for(j in 1:length(starts)) { ## because there may be more than one match
#       if(starts[j] <1) {starts[j] <- 1}
#       range <- append(range,starts[j]:ends[j])
...
```{r index criteria on mutant gene pattern}
...
# 
# 
# eligibilities$change[grep(pattern = fusionPattern, x=eligibilities$criteria, ignore.case = T)] <- "FUSION"

 
# ## NOW eligibilities TABLE JUST CONTAINS INDIVIDUAL  CRITERIA (1 CRITERION PER ROW), INDEXED BASED ON PATTERNS SPECIFIED
...
fusion.eligibilities <- eligibilities
...
## define which rows contain a match for mutantPattern
fusionIndices <- grep(pattern = fusionPattern, x=fusion.eligibilities$criteria, ignore.case = T)
# subset table 
fusion.eligibilities <- fusion.eligibilities[fusionIndices, ]
# label nature of change
fusion.eligibilities$feature <- "FUSION"
...
## join to Entrez symbols from humanGenes
fusion.eligibilities <- merge(x=fusion.eligibilities, by.x="match", y=humanGenes, by.y = "Aliases")
...
## loop through each criterion that contains a fusion pattern
# for(i in 1:nrow(eligibilities)) {
#   if(!is.na(eligibilities$change[i]) & eligibilities$change[i]=="FUSION") {
#     criterion <- eligibilities$criteria[i]
#     
#     # split into tokens
#     criterion <- unlist(strsplit(x=criterion, split = " |\\(|\\)|,|-|\\/"))
#     
#     
#     ## get indices of match(es) against fusionPattern
#     indices <- grep(pattern = str_squish(fusionPattern), x = criterion, ignore.case = TRUE)
#     ## specify start and end point(s) based on specified word range
...
```{r row bind indexed eligbility table}
...
```{r preview indexed eligibilities table}
formattable(head(indexed.eligibilities,40))

## ACTUALLY - BETTER TO MAKE SEPARATE SUBTABLES FOR EACH OF MUTATIONS AND FUSIONS, THEN ROW BIND THEM TOGETHER (and can extend to cover indexing on other features later if needed)
