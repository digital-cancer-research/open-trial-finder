...
## inner join
eligibilities_CMD <- unique(merge(x=eligibilities_CMD, by.x = "tokens", all.x = FALSE, y = unique(dplyr::select(NCIt_cellMolecularDysfunction, Synonyms_stem, ID,PreferredTerm, Class)), by.y = "Synonyms_stem"))

eligibilities_CMD$vocab <- "NCIt"

## select and rename columns to match eligibility indices
eligibilities_CMD <- dplyr::select(eligibilities_CMD, criterion_sentence_index, "matching_token"="tokens", "topic" = "Class", "PT" = "PreferredTerm", vocab, "vocab_id" = "ID")
