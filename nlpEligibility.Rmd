...
# eligibilities_temp$sentences_stem <- textstem::stem_strings(eligibilities_temp$sentences)
...
```{r sentiment analysis of eligibility criteria}
# tic("sentiment analysis of all criteria")
# eligibilities$sentiment_score <- analyzeSentiment(eligibilities$criteria)$SentimentQDAP
# toc()
...
```{r load humanGenes}
humanGenes <- dbGetQuery(con, statement = "SELECT * FROM humanGenes")
...
## drop any rows where synonyms stem is a single character
## (e.g. "r" is a synonym of C3360)
NCIt <- NCIt[nchar(NCIt$Synonyms_stem)>1, ]
...
```{r define tidy format for index data}
eligibility_indices <- data.frame(criterion_sentence_index = character(0), 
                                  matching_token = character(0),
                                  topic = character(0), 
                                  PT = character(0), 
                                  vocab = character(0), 
                                  vocab_id = character(0))
...
```{r index on genes}
## WE WILL ONLY MATCH ON GENE NAMES THAT ARE SINGLE WORDS 
## (otherwise, we would have to supply 124,848 aliases as custom tokens, and this throws an error - protection stack overflow... 
## we will perform case-sensitive matching (using case as in humanGenes dataframe)
## we will not use the stemmed version
eligibilities_genes <- dplyr::select(eligibilities, criterion_sentence_index, sentences) %>%
  unique() %>%
  tidytext::unnest_tokens(tokens, sentences, token="words", to_lower=FALSE) 

eligibilities_genes <- unique(merge(x=eligibilities_genes, by.x="tokens", y=humanGenes, by.y = "Aliases"))

eligibilities_genes$topic <- "Gene"
eligibilities_genes$vocab <- "Entrez"
eligibilities_genes <- unique(dplyr::select(eligibilities_genes, criterion_sentence_index, "matching_token"="tokens", topic, "PT"="Symbol", vocab, "vocab_id"="GeneID"))

## row-bind onto eligibility_indices table
eligibility_indices <- unique(rbind(eligibility_indices, eligibilities_genes))

## delete eligibilities_genes from memory
rm(eligibilities_genes)
```

### **Index on Cell or Molecular Dysfunction**  
...
## we will match stemmed NCIt synonyms against stemmed tokens
custom_tokens <- unique(NCIt_cellMolecularDysfunction$Synonyms_stem)

## select required columns
eligibilities_CMD <- unique(dplyr::select(eligibilities, criterion_sentence_index, sentences, sentences_stem)) 


## tokenise
eligibilities_CMD$tokens <- as.list(corpus::text_tokens(x=eligibilities_CMD$sentences_stem,                                                filter= corpus::text_filter(combine = custom_tokens, map_case=FALSE, connector="_", drop_punct=TRUE )))

## unnest
eligibilities_CMD <- as.data.frame(unnest(data = eligibilities_CMD, tokens)) 

## replace underscores with spaces
eligibilities_CMD$tokens <- gsub(pattern = "_", replacement = " ", x=eligibilities_CMD$tokens)



## inner join
eligibilities_CMD <- unique(merge(x=eligibilities_CMD, by.x = "tokens", all.x = FALSE, y = unique(dplyr::select(NCIt_cellMolecularDysfunction, Synonyms_stem, ID,PreferredTerm, Parent_synonym,Class)), by.y = "Synonyms_stem"))

eligibilities_CMD$vocab <- "NCIt"

## select and rename columns to match eligibility indices
eligibilities_CMD <- dplyr::select(eligibilities_CMD, criterion_sentence_index, "matching_token"="tokens", "topic" = "Class", "PT" = "Parent_synonym", vocab, "vocab_id" = "ID")


## row-bind onto eligibility_indices table
#eligibility_indices <- unique(rbind(eligibility_indices, eligibilities_CMD))

## delete eligibilities_CMD from memory
#rm(eligibilities_CMD)
...
See also: https://www.tidytextmining.com/ngrams.html  
  
We can convert each criterion into a *Markov chain*.  
  
```{r define example criteria}
...
```{r tokenise criteria into bigrams}
