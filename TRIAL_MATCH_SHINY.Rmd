...
observe({
      ...
      if (!is.null(query[['alterations']])) {
        ...
        # urlValues$alterations <- unlist(strsplit(urlValues$alterations, split=","))
...
#       query <- parseQueryString(session$clientData$url_search)
#       if (!is.null(query[['alterations']])) {
#         urlAlterations <- query[['alterations']]
#         updateTextInput(session, "alterations", value = urlAlterations)
#       }
#     })
...
## or http://127.0.0.1:5793/TRIAL_MATCH_SHINY.Rmd?cancerType=Breast&alterations=ERBB2%20amplification,%20KRAS%20mutation
...
selectedAlterations <- reactive({
  if(!is.null(urlValues$alterations)) {
    alterations <- urlValues$alterations
  } else {alterations <- input$geneFilter}
...
output$info = renderPrint({
    list(rows = input$datatable_rows_selected)
  })
