...
output$report <- downloadHandler(
      ...
      content = function(file) {
        ...
        params <- list(subject_ID="not provided", 
                       ...
                       table=toJSON(reactiveCancerStudies())
                       )
...
reactiveCancerStudies <- reactive({
   ...
   } #else {table[ , c("rationale")] <- NA} #else {table[ , c("gene.of.interest","intervention.name","intervention.target","rationale","confidence","criteria")] <- NA}  ## add empty columns so can select and rename for display...
...
shiny::renderUI({
   ...
   if(nrow(table)>0) {
   HTML(table %>%
            mutate(nct_id = cell_spec(x=table$nct_id,format="html", link = table$Link, new_tab = TRUE )) %>%
   ...
   #         dplyr::select("Matching cancer type" = "matching.condition",
   #                       "Matching gene" = "gene.of.interest",
   #                       "Rationale" = "rationale",
   #                       "Brief title" = "brief_title",
   #                       "Matching inclusion criterion" = "criteria",
