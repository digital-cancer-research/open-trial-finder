...
urlAlterations <- reactive({
    ...
    # ## if none of the variants passed is in scoredMatches table, set value to NULL
    # if(length(intersect(alterations, scoredMatches$gene_variant_type))==0) {
    #   alterations <- NULL
    # }
...
selectedAlterations <- reactive({
  ...
  ## if URL alterations but nothing on geneFilter, URL alterations are used
  # if(!is.null(urlAlterations()) & is.null(input$geneFilter)) {
  #   alterations <- urlAlterations()
  #   ## reset urlAlterations value
  #   urlAlterations() <- NULL
  # } else if(!is.null(input$geneFilter)) {   ## if geneFilter is not null, use those
  #   alterations <- input$geneFilter
  # #} else {alterations <- NULL}
  # } else if(is.null(urlAlterations()) & is.null(input$geneFilter)) {
  #   alterations <- NULL}
...
reactiveScoredMatches <- reactive({
   ...
   ## print to console to aid development... 
   #print(paste0("head of reactiveScoredMatches1: ", head(table)))
...
output$downloadShortlist <- downloadHandler(
      ...
      content = function(file) {
        ...
        ## capture the alterations (if any) that have been passed in url
        #print(paste0("urlAlterations() value: ", urlAlterations()))
        #print(paste0("urlValues$alterations value: ", urlValues$alterations))
        shortlistTable$urlAlterations <- urlAlterations
        # if(!is.null(urlValues$alterations)) {
        #   shortlistTable$urlAlterations <- urlValues$alterations
        # }
        ## print to console to aid development... 
        #print(shortlistTable)
