...
#scoredMatches <- scoredMatches[order(scoredMatches$confidence, decreasing = TRUE), ]
```
...
output$studiesmap <- renderLeaflet({
   ...
   if(nrow(locations)>0) {
     leaflet() %>%
         ...
         setView(lat = configuration$zoom.lat, lng = configuration$zoom.long, zoom = 5) %>%
     ...
     leaflet() %>%
         addProviderTiles(providers$OpenStreetMap.Mapnik) %>%
         setView(lat = 54.2361, lng = -4.5481, zoom = 6)
  }
})
