...
```{r render reactive datatable selectable cells catch error for empty}
...
#         table <- aggregate(table, by=table['nct_id'], function(x) {paste(unique(x), collapse = ";\n")})
#       ## add checkboxes 
#       ## OPTION 1,  based on https://github.com/rstudio/DT/issues/93#issuecomment-111001538
#       # create a character vector of shiny inputs
#       shinyInput = function(FUN, len, id, ...) {
#         inputs = character(len)
#         for (i in seq_len(len)) {
#           inputs[i] = as.character(FUN(paste0(id, i), label = NULL, ...))     }
#         return(inputs)  } # end shinyInput function
#       ## call function to add a column of checkboxes
#       table$Shortlist = shinyInput(FUN=checkboxInput, len = nrow(table), id='Shortlist_', value = FALSE)
#       
#       ## OPTION 2, based on https://github.com/rstudio/DT/issues/185
#       # add a checkbox column
#       # table$Shortlist <- paste0("<input id='type1_",1:nrow(table), "' type='checkbox' >")
#       # ## to get the selected rows see https://yihui.shinyapps.io/DT-radio/ 
# 
#       ## OPTION 3
#       # action buttons in column of table
#       # table$Shortlist = buttonInput(
#       #           FUNCTION = actionButton,
#       #           length = nrow(table),
#       #           id = 'button_',
#       #           label = "Add to shortlist",
...
#       #           onclick = 'Shiny.onInputChange(\"lastClick\",  this.id)'
#       #         )
#       ## OPTION 4, based on https://shiny.rstudio.com/articles/action-buttons.html 
#       # table$Shortlist = buttonInput(
#       #           FUNCTION = actionButton,
#       #           length = nrow(table),
#       #           id = 'do',
...
#                                ))
#         
#         ## add a column with shortlisting info
#         #table$Shortlist <- "Add to shortlist"
#         table <- arrange(table, desc(combined_score))
#         table <- dplyr::select(table, -combined_score)
#         
#         
...
# 
# 
# # obtain the values of e.g. Shortlist inputs
# shinyValue = function(id, len) {
#       unlist(lapply(seq_len(len), function(i) {
#         value = input[[paste0(id, i)]]
#         
#         if (is.null(value)) {return(NA)} else {return(value)}
#       }))
#     }
# 
# 
# 
# 
# output$datatable <- DT::renderDT(dplyr::select(formattedTable(), -ID), rownames=FALSE, escape = 5, options = list(ordering=F,                                                                                           paging = FALSE,                                                                   scrollX = TRUE, 
#       scrollY = "450px",                                                                          scrollCollapse = TRUE, 
#       autoWidth = TRUE,
#       columnDefs = list(list(width = '500px', targets = list(5)))
#       #columnDefs = list(list(targets=c(5), visible=TRUE, width='30%'))
#   )) 
...
fillCol(height = 600, flex = c(NA, 1), 
        ...
        inputPanel(shiny::renderUI({
          ...
          shiny::renderUI({
          actionButton("addToShortlist", "Shortlist selected studies", class = "btn-dark")
          })
          ),
        DT::DTOutput("datatable") 
        )
...
## print indices of selected rows to markdown console
# observeEvent(input$datatable_rows_selected, {
#     print(input$datatable_rows_selected)
#   })

## when shortlist button is clicked, print indices of selected rows to markdown console
observeEvent(input$addToShortlist, {
    #print(input$datatable_rows_selected)
  print(indicesSelectedRows())
  })
...
```{r function to add selected rows to shortlist }
...
#   observeEvent(input$addToShortlist, {
#     print(indicesSelectedRows())
#     table <- formattedTable()[indicesSelectedRows(), ]
#   })
#   
#   
#   
...
## add ellipsis to locations, lose ID and contacts columns
# output$shortlistTable <- DT::renderDT(dplyr::select(shortlist(), -c(ID,Contacts)), escape = 5, options = list(ordering=F, paging = FALSE, scrollX = TRUE, scrollY = "450px", scrollCollapse = TRUE, columnDefs = list(list(
#   targets = 6,
#   render = JS(
#     "function(data, type, row, meta) {",
#     "return type === 'display' && data.length > 6 ?",
...
## no ellipsis, keep contacts
# output$shortlistTable <- DT::renderDT(dplyr::select(shortlist(), -ID), escape = 5, options = list(ordering=F, paging = FALSE, scrollX = TRUE, scrollY = "450px", scrollCollapse = TRUE) )

## increase width of brief title column
output$shortlistTable <- DT::renderDT(dplyr::select(shortlist(), -ID), rownames=FALSE, escape = 4, options = list(ordering=F,                                                                                           paging = FALSE,                                                                   scrollX = TRUE, 
      scrollY = "450px",                                                                          scrollCollapse = TRUE, 
      autoWidth = TRUE,
      columnDefs = list(list(width = '500px', targets = list(4)))
      #columnDefs = list(list(targets=c(5), visible=TRUE, width='30%'))
...
fillCol(height = 600, flex = c(NA, 1), 
        ...
        inputPanel(
          shiny::renderUI({downloadButton("downloadShortlist", "Download selected studies", class = "btn-primary")})
