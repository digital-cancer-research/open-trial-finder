...
head(summary(eligibilities_corpus))

## Change the unit of texts to sentences
# eligibilities_corpus <- corpus_reshape(eligibilities_corpus, to = "sentences")
# head(summary(eligibilities_corpus))

## change back to criteria
# eligibilities_corpus <- corpus_reshape(eligibilities_corpus, to = "documents")
# head(summary(eligibilities_corpus))
...
eligibilities_tokens <- tokens(eligibilities_corpus, remove_punct=TRUE)
...
## can use (big) lists of patterns
kw_genes <- kwic(eligibilities_tokens, pattern =  unique(humanGenes$Aliases), window = 5, case_insensitive = FALSE)
...
NOT USED - PERFORM THIS SEPARATELY, AS TOO MEMORY-INTENSIVE....  
