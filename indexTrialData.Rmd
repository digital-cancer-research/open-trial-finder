...
cancerStudiesQ <- paste0("SELECT s.nct_id, s.brief_title, s.primary_completion_date, s.last_update_posted_date, s.phase, s.acronym, s.number_of_arms, s.number_of_groups,  s.overall_status, s.last_update_posted_date, c.name AS condition, f.city, f.status AS site_status, fc.email AS site_contacts, cc.email AS central_contacts
FROM studies s
...
eligibilities_alterations_genes <- eligibilities %>%
    ...
    tokens() %>%
    kwic(pattern =  alterations_dictionary, window = window_size) %>% as.data.frame() %>% 
...
eligibilities <- eligibilities %>%
  ...
  dplyr::select(nct_id, criterion_index, criterion_type, criteria, criteria_processed, geneSymbol, alteration_type) %>%
  unique()
...
## only 4 exclusion criteria identified (and one of them is wrong) - pattern is too stringent?

## use list instead of regex -> case-insensitive kwic... 
eligibilities %>%
    dplyr::filter(criterion_type == "EXCLUSION") %>%
    dplyr::select(nct_id, criterion_index, criteria) %>%
    unique() %>%
    corpus(docid_field = "criterion_index",  text_field = "criteria") %>% 
    tokens() %>%
    kwic(pattern =  list("previous", "prior"), window = window_size) %>% 
    dplyr::mutate(window = paste0(pre, " ", keyword, " ", post)) %>% 
    dplyr::mutate(therapy_code = as.list(tokens_lookup(x=tokens(window), dictionary = therapy_dictionary, nested_scope = "dictionary"))) %>%
    unnest(cols = "therapy_code") %>%
    as.data.frame() %>%
...
## consider omitting requirement for "prior", to include concomitant therapy exclusions... 
## use list instead of regex -> case-insensitive kwic... 
# eligibilities %>%
#     dplyr::filter(criterion_type == "EXCLUSION") %>%
#     dplyr::mutate(therapy_code = as.list(tokens_lookup(x=tokens(criteria), dictionary = therapy_dictionary, nested_scope = "dictionary"))) %>%
#     unnest(cols = "therapy_code", keep_empty = FALSE) %>%
#     as.data.frame() %>%
...
prior_tx_pattern = list("prior", "previous", "concurrent", "concomitant")
