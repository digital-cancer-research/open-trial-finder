...
```
  ...
  * Assign *stratification_score* +3 if study design group includes patients with 1 or more specified alteration  
...
## create a string to describe rationale
scored_matches_dg$stratification_rationale <- paste0("Stratified on ", scored_matches_dg$gene_variant_type)

## select, rename and reorder columns for compatibility with trialMatchDataRefresh.Rmd
scored_matches_dg <- unique(dplyr::select(scored_matches_dg, "symbol"="controlled_match", "variant_type"="feature", nct_id, stratification_rationale, text))

## where multiple criteria, aggregate
scored_matches_dg <- scored_matches_dg %>%
                                      group_by(nct_id, symbol, variant_type, stratification_rationale) %>%
                                      summarise(
...
## add a score
scored_matches_dg$stratification_score <- 3
```
