...
```{r filter and retain only result groups for outcomes and events}
...
#### **Assign interventions based on mentions of study interventions**  
...
# descign group intervention descriptions = different between groups
...
create_dtm <- function(dataframe, text_column, id_column, custom_tokens) {
  ## remove punctuation... (e.g. &, =) from the text column
  # dataframe[ , which(names(dataframe)==text_column)] <- str_squish(str_replace_all(string = dataframe[ , which(names(dataframe)==text_column)], "[^[:alnum:]]", replacement = " "))
  ## remove anything that is not a number or letter...
  dataframe[ , which(names(dataframe)==text_column)] <- str_squish(str_replace_all(string = dataframe[ , which(names(dataframe)==text_column)], "[^a-zA-Z0-9]", replacement = " "))
  ...
  dataframe$word <- as.list(corpus::text_tokens(x=dataframe[ , which(names(dataframe)==text_column)],                                                filter= corpus::text_filter(combine = custom_tokens, map_case=TRUE, connector="_", drop_punct=TRUE )))
  ## unnest
