...
```{r NOT NEEDED download and process a list of all human genes and their synonyms}
...
```{r NOT NEEDED save table of human genes to DB}
...
```{r NOT NEEDED download and create NCIthesaurus table}
# ## start timer 
...
```{r NOT NEEDED write NCIt to database}
## create as a table in database
...
eligibilities <- eligibilities %>% 
  ...
  dplyr::mutate(criterion_index = paste0(nct_id, "_", row_number())) %>% 
  as.data.frame()
```
...
postcodesURL <- "https://data.freemaptools.com/download/full-uk-postcodes/ukpostcodes.zip"    ## as of May 2023
...
tryCatch(
    { download.file(url=postcodesURL, destfile = destPostcodesFilename)
        unzip(zipfile = normalizePath(destPostcodesFilename))    ## as of May 2023
...
* In order to reduce the number of joins in the Shiny app, we will join prior therapy exclusion criteria to the open cancer studies view before saving to DB (see below)...   
...
* As of `r format(Sys.Date(), format = "%d %b %Y")`:  
  ...
  * the mean number of days since the study records were updated is `r mean_days_since_update`  ```{r save open_cancerStudies_view to DB}
## preview
formattable(head(open_cancerStudies_view))


```{r join prior tx exclusions to open_cancerStudies_view}

## filter for exclusions
excludedTX <- dplyr::filter(indexed_eligibilities_view, criterion.type=="EXCLUSION")
...
## join to open_cancerStudies_view

open_cancerStudies_view <- plyr::join(x=open_cancerStudies_view, y=excludedTX, by = "nct_id", type = "left")    ## plyr::join is faster than merge

```



````{r write studies view to DB}
## create as a table in database
