...
cancerStudiesQ <- paste0("SELECT s.nct_id, s.brief_title, s.primary_completion_date, s.last_update_posted_date, s.phase, s.acronym, s.number_of_arms, s.number_of_groups,  s.overall_status, s.last_update_posted_date, c.name AS condition
FROM studies s
...
cancerStudies$cancertype_pt <- cancerStudies$condition %>%
...
cancerStudies <- cancerStudies %>%
  unnest( cols = "cancertype_pt", keep_empty = TRUE) %>% 
  as.data.frame()

## keep detail of unmapped condition names... 
unmapped_condition_names <- unique(cancerStudies$condition[is.na(cancerStudies$cancertype_pt) ])

## now delete rows that haven't been mapped
cancerStudies <- cancerStudies %>%
  dplyr::filter(!is.na(cancertype_pt))
...
interventions <- interventions %>%
  ...
  dplyr::select(nct_id, intervention_id, intervention_name, NCIt_code, intervention_pt,TARGET_SYMBOL, Mechanism) %>%
  unique()
...
interventions <- interventions %>%
    merge(by.x = c("nct_id", "intervention_id"), all.x = TRUE, y = dplyr::select(dgi, nct_id, design_group_id, intervention_id), by.y = c("nct_id", "intervention_id")) %>%
    unique()
...
eligibilities <- eligibilities %>%
  ...
  dplyr::select(nct_id, criterion_index, criteria, criteria_processed, geneSymbol, alteration_type) %>%
  unique()
