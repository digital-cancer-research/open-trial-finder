...
```{r define result groups to be mapped}
## we want to predict which interventions were given for each result group
## if a study has only a single arm, we will assume all study interventions were administered for all result groups

number_arms <- unique(dplyr::select(cancerStudies, nct_id, number_of_arms))
rgs <- unique(dplyr::select(result_groups, nct_id, rg_id, rg_title, rg_description))
rgs$rg_title_description <- paste(rgs$rg_title, rgs$rg_description, sep = ": ")
rgs <- unique(dplyr::select(rgs, nct_id, rg_id,rg_title, rg_title_description))
rgs <- merge(x=number_arms, by.x="nct_id", y=rgs, by.y = "nct_id")

rgs$rg_interventions <- NA 
rgs$rg_intervention_ids <- NA
...
synonym2preferred <- function(synonym) {
  ...
  preferred <- NCIt_drugs_preferred$PreferredTerm[NCIt_drugs_preferred$Synonyms_lower==synonym] ## get the preferred term for that synonym
