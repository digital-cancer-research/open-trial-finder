...
UKfacilitiesQ <- paste0("SELECT f.nct_id, f.status AS facility_status, f.name AS facility_name, f.city, f.zip, fi.name AS investigators, cc.email AS central_contacts, fc.email 
FROM facilities f
...
## select and reorder columns
# uk_facilities <- unique(dplyr::select(uk_facilities, nct_id, facility_name, facility_status, city, latitude, longitude))
...
open_cancerStudies_view <- unique(merge(x=open_cancerStudies_view, 
                                   by.x = "nct_id", 
                                   y = dplyr::select(uk_facilities, nct_id, "site_name"="facility_name", "site_status" = "facility_status", "contacts" = "email", central_contacts, investigators, "locations"="city","postcode"="longitude", "postcode.lat"="latitude", "postcode.long"="longitude", "lat"="latitude", "long"="longitude"), 
                                   by.y = "nct_id"))
...
open_cancerStudies_view <- unique(dplyr::select(open_cancerStudies_view, interventions, locations, postcode, nct_id, brief_title, overall_status,"condition"="matching.condition", site_name, site_status, investigators, contacts, central_contacts, Refresh.date, matching.condition, TARGET.condition, Link, postcode.lat, postcode.long, lat, long, ParentTerm))
```
