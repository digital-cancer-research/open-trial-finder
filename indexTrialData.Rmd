...
humanGenes <- humanGenes[!humanGenes$Aliases %in% c("I", "II", "III", "IV", "V", "VI", "VII", "VIII"), ]
...
eligibilities_tokens <- tokens(eligibilities_corpus, remove_punct=TRUE, padding = FALSE)
toc()
```
...
kwic_genes <- as.data.frame(kwic(eligibilities_tokens, pattern =  unique(humanGenes$Aliases), window = 6, case_insensitive = FALSE))
toc()
...
kwic_genes <- merge(x=dplyr::select(eligibilities, nct_id, criterion_index, criteria, criterion_type), by.x="criterion_index", y=kwic_genes, by.y="criterion_index")

## rename and reorder columns
kwic_genes <- unique(dplyr::select(kwic_genes, nct_id, criterion_index, criterion_type,criteria, context, "match"="gene_synonym", "controlled_match"="Symbol"))
...
indexOnPattern <- function(dataframe, pattern, featureLabel) {
  ...
  indices <- grep(pattern = pattern, x=kwic_genes$context, ignore.case = TRUE)
  ## subset dataframe
...
```{r index on mutations}

## define mutant pattern
mutant_pattern <- "mutat|mutant|defect|deficien|alter|loss of function|loss-of-function"

mutation_eligibilities <- indexOnPattern(dataframe = kwic_genes, pattern = mutant_pattern, featureLabel = "mutation")

## select and reorder columns for consistency with trialMatchDataRefresh
mutation_eligibilities <- unique(dplyr::select(mutation_eligibilities, match, nct_id, criteria, criterion_type, feature, controlled_match ))
```

```{r index on rearrangements}

rearrangement_eligibilities <- indexOnPattern(dataframe = kwic_genes, pattern = " fusion|rearrangement|truncation|truncated|deletion|deleted|lost|duplication|duplicated|trasloc", featureLabel = "rearrangement")
...
loss_eligibilities <- indexOnPattern(dataframe = kwic_genes, pattern = " loss", featureLabel = "loss")
...
amplification_eligibilities <- indexOnPattern(dataframe = kwic_genes, pattern = " amplifi|overexpress", featureLabel = "amplification")
...
therapy_eligibilities <- eligibilities[grep(pattern = "previous therapy|prior therapy|previous treatment|prior treatment", x=eligibilities$criteria, ignore.case = TRUE), ]
