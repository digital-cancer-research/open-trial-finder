...
```{r copyright notice}
 ...
 # Copyright (C) 2021-2023 digital ECMT
 # 
...
* Dictionaries are used to lookup and map terms from ct.gov study record to preferred terms.  
  
#### **Human gene names and synonyms**  
  
* Entrez Gene Symbols are used as preferred terms for gene names.  
  
```{r download and process a list of all human genes and their synonyms, warning=FALSE, message=FALSE}  
...
```{r remove KRAS as synonym of NRAS}
humanGenes <- humanGenes %>%
    dplyr::filter(!(Symbol == 'NRAS' & Aliases == 'KRAS'))

```

```{r include ARID1a as synonym of ARID1A}

humanGenes <- humanGenes %>%
    dplyr::add_row(GeneID = '8289', Symbol = 'ARID1A', Aliases = 'ARID1a')
```


#### **Drug names and synonyms**  
  
* The NCI thesaurus is used as a source of preferred terms and synonyms for drugs, plus parent-child relationships between terms  

```{r download NCI thesaurus, message=FALSE}
...
tryCatch(
  expr = {
    ...
    invisible(file.remove("NCIt_FLAT.zip"))
  }, error = function(e) {
...
## as of July 2024, only 8 columns are included in download, 'concept in subset' appears to be missing (although this is not as stated in readme file at https://evs.nci.nih.gov/ftp1/NCI_Thesaurus/archive/24.06d_Release/ReadMe.txt)
# names(NCIt) <- c("code", "concept_IRI", "parents", "synonyms", "definition", "display_name", "concept_status", "semantic_type")


```

```{r  normalise NCI thesaurus}
...
```{r exclude uninformative entities from NCIt}
...
NCIt_multiple_terms <- NCIt %>%
  ...
  dplyr::summarise(
    number_terms = length(base::unique(code))
  ) %>%
...
NCIt_Pharmacologics <- base::unique(dplyr::filter(NCIt, semantic_type %in% c("Immunologic Factor", "Pharmacologic Substance")))

```
  
#### **Drug-target relationships**  
  
* Drug-target relationships are primarily sourced from ChEMBL  
...
* Therefore, IUPHAR is used as an additional source of drug-target relationships  
...
drugs_targets <- base::unique(rbind(drugs_targets, iuphar_processed))

## check e.g. for Everolimus (C48387)
#dplyr::filter(drugs_targets, NCIt_code == "C48387")
```

#### **Preferred terms and synonyms for cancer types**  
  
* Preferred terms and synonyms for cancer types are based on a user-defined term set  
  * Loaded from *conditionSynonyms6.csv* file  
...
simple_conditions <- data.frame(controlled_cancer_type = base::unique(conditionSynonyms$controlled_cancer_type), condition_synonyms = base::unique(conditionSynonyms$controlled_cancer_type))
## second, bind this to the conditionSynonyms table
conditionSynonyms <- base::unique(rbind(conditionSynonyms, simple_conditions))
...
conditionSynonyms <- base::unique(conditionSynonyms)
...
* Study records are sourced exclusively from clinicaltrials.gov, via AACT   
  * A user name and password for an AACT account is required.  
  * These must be specified in the *trialMatchConfiguration.json* file  
...
* Only cancer studies with at least one site open in one of the countries specified in *trialMatchConfiguration.json* file are indexed  

```{r get data for open cancer studies in configured countries}

## format country names for SQL query
#studyIDsForSQL <- paste0("\'",paste(base::unique(cancerStudies$nct_id), collapse = "\',\'"), "\'")

countries_for_sql <- configuration$country %>%
    stringr::str_split(pattern = ",") %>%
    unlist() %>%
    stringr::str_squish() %>%
    sprintf(fmt = "'%s'", .) %>%      ## surround countries with single quotes
    toString()  %>%       ## convert to string
    tolower()


## get data for all cancer studies that have reported results
cancerStudiesQ <- paste0("SELECT s.nct_id, s.brief_title, s.primary_completion_date, s.last_update_posted_date, s.phase, s.acronym, s.number_of_arms, s.number_of_groups,  s.overall_status,  c.name AS condition, f.city, f.country, f.status AS site_status, fc.email AS site_contacts, cc.email AS central_contacts
FROM studies s
...
AND lower(f.country) IN (", countries_for_sql, ")
...
* As of `r today`, for `r configuration$country` this includes **`r length(base::unique(cancerStudies$nct_id))` cancer studies**.  
  
#### **Map condition names to preferred terms for cancer types**  
  
* condition names (*conditions.name*) are mapped to cancertype preferred terms using dictionary lookup  
...
unmapped_condition_names <- base::unique(cancerStudies$condition[is.na(cancerStudies$condition_PT) ])
...
* **The following condition names have not mapped to preferred terms and are lost at this stage**:  
  
`r unmapped_condition_names`  
  
* After mapping to preferred terms for cancer types, **`r length(base::unique(cancerStudies$nct_id))` studies remain**  
...
#### **Get latitude and longitude values for sites**  
  
* City name will be used to get latitude and longitude values to display site locations in user interface  
  
```{r get lat and long values for cities}

cancerStudies <- cancerStudies %>%
    # dplyr::mutate(city_country = paste0(city, ", ", configuration$country)) %>%
    dplyr::mutate(city_country = paste0(city, ", ", country)) %>%
...
#### **Map intervention names to preferred terms**  
  
* study interventions (*interventions.name*) are mapped to NCI thesaurus entities, display_name values are used as preferred terms  
...
* parent terms are joined from NCI thesaurus - for drugs, these are typically labels for the relevant drug class  
...
#### **Index inclusion criteria on genetic alterations**  
  
* Inclusion criteria are indexed based on co-occurrence of an alteration synonym within a 6-word window of a human gene name  
  * To improve precision, only gene names included in the NCI thesaurus (i.e. gene relevant to cancer) are included  
...
* Such patterns are expanded during preprocessing so that e.g. *BRCA1/2* is converted to *BRCA1 or BRCA2*  
...
eligibilities_alterations_genes <- eligibilities %>%
    dplyr::filter(criterion_type == "INCLUSION") %>%
...
* Similarly, exclusion criteria are indexed based on co-occurrence of the word "prior" or "previous" within a six-word window of a treatment synonym  
...
prior_tx_exclusions <- eligibilities %>%
    ...
    dplyr::group_by(nct_id) %>%
    dplyr::summarise(
        tx_exclusions = paste(base::unique(criteria), collapse = ";\n")
    ) %>%
    as.data.frame() %>%
    base::unique()
...
#### **Index design groups on genetic alterations**  
  
* Design group (i.e.treatment arm) titles and descriptions are indexed in same way as inclusion criteria  
...
if(length(replacement_strings) > 0) {
  ## replace all occurrences of the verbatim string with the expanded string
...
if(length(replacement_strings2)>0) {
  ## replace all occurrences of the verbatim string with the expanded string
design_groups$dg_title_description_processed <- design_groups$dg_title_description_processed %>%
  stringr::str_replace_all(pattern = replacement_strings2)
}
...
scored_matches <- eligibilities %>%
  ...
  dplyr::summarise(
    matching_criteria = paste(base::unique(criteria), collapse = "\n\n")
    ) %>%
  as.data.frame() %>%
...
scored_matches <- design_groups %>%
  ...
  dplyr::group_by(nct_id, gene_alteration_type) %>%
  dplyr::summarise(
    matching_criteria = paste(base::unique(dg_title_description), collapse = "\n\n")
    ) %>%
  as.data.frame() %>%
...
* Assign *intervention_score* +2 to a study if any intervention targets an altered gene directly  
  * No intervention scores are assigned if the alteration type is *loss* (since the drug target will not be present)  
  * For amplifications, positive modulators (e.g. agonsts, activators etc) are not assigned a score  
  
```{r specify MOAs relevant for each alteration type}

action_types_list <- list(base::unique(na.omit(interventions$action_type)))
...
scored_matches_interventions <- interventions %>% 
  ...
  dplyr::group_by(nct_id, gene_alteration_type) %>%
  dplyr::summarise(
    intervention_rationale = paste(base::unique(intervention_rationale), collapse = "\n")
  ) %>%
  as.data.frame() %>%
...
tryCatch(
  expr = {
    ...
    kegg_edges <- base::unique(dplyr::select(pathwaysInCancer_edges, "upstream"="entry1symbol", "downstream"="entry2symbol"))
...
scored_matches_interventions_downstream <- interventions %>%
  ...
  dplyr::summarise(
    TARGET_SYMBOL = paste(base::unique(TARGET_SYMBOL), collapse = ",")
  ) %>%
  as.data.frame() %>%
...
* Calculate a *combined_score* = *eligibility_score* + *intervention_score*  
  * The *combined_score* value is used to rank studies in the Shiny app  
...
interventions <- interventions %>%
  dplyr::select(nct_id, intervention_name, intervention_pt, NCIt_code, TARGET_SYMBOL, action_type, Parent_synonym) %>%
  base::unique()
...
* interventions.nct_id: study identifier  
* interventions.intervention_name: verbatim intervention name, as stored in ct.gov record  
* interventions.intervention_pt: preferred term for intervention, based on NCI thesaurus display_name, if available  
* interventions.NCIt_code: NCI thesaurus code for study intervention  
* interventions.TARGET_SYMBOL: Entrez symbol for molecular target of drug, if available  
* interventions.action_type: action of intervention on target, if available  
* interventions.Parent_synonym: synonyms of intervention's parent term in NCI thesaurus  
  
  
  
#### **scored_matches table**  
  
```{r preview scored_matches table}
...
* scored_matches.nct_id: study identifier  
* scored matches.gene_alteration_type: genetic alteration that has been linked to study, format = <*Entrez symbol*> <*alteration type*>  
* scored_matches.eligibility_rationale: short text description describing rationale linking study to genetic alteration, if applicable  
* scored_matches.intervention_rationale: short text description describing rationale linking study intervention to genetic alteration, if applicable  
* scored_matches.matching_criteria: verbatim text from study record (inclusion criteria/treatment group description) linking study to genetic alteration, if applicable  
* scored_matches.combined_score: sum of eligibility and intervention scores for that combination of study and genetic alteration  
  
  
#### **conditionSynonyms table**  
  
```{r preview conditionSynonyms table}
...
* conditionSynonyms.controlled_cancer_type: user-specified set of preferred terms for cancer types  
* conditionSynonyms.condition_synonyms: user-specified set of synonyms for each cancer type, used for dictionary lookup against condition.name values in ct.gov study record  
