...
cancer_drugs <- unique(dplyr::select(cancer_drugs, "drug_name"="Product", "EMA_approved"="EMA", "FDA_approved"="FDA", "DrugBank_link"="DrugBank.ID", Indications, Targets))

## Note, drugs such as Inavolisib don't appear in this list, but do have entries in KEGG... 

## map to NCIt IDs
cancer_drugs$drug_name_lower <- tolower(cancer_drugs$drug_name)
NCIt$Synonyms_lower <- tolower(NCIt$Synonyms)

## for combinations, drug names are comma-separated - we will need to split and unnest in order to map to NCIt... 

cancer_drugs$drug_name_lower <- strsplit(x=cancer_drugs$drug_name_lower, split = ", ")
cancer_drugs <- unnest(data = cancer_drugs, cols = drug_name_lower, keep_empty = TRUE) 
cancer_drugs <- as.data.frame(cancer_drugs) ## convert to data frame
## join to NCIt ID
cancer_drugs <- unique(merge(x=cancer_drugs, by.x="drug_name_lower", all.x = TRUE, y=unique(dplyr::select(NCIt, "NCIt_ID" = "ID", Synonyms_lower)), by.y="Synonyms_lower"))

cancer_drugs <- dplyr::select(cancer_drugs, -drug_name_lower)

## unnest the Targets column
cancer_drugs$Targets <- strsplit(x=cancer_drugs$Targets, split = "; ")
cancer_drugs <- unnest(data = cancer_drugs, cols = Targets, keep_empty = TRUE) 
cancer_drugs <- as.data.frame(cancer_drugs) ## convert to data frame
...
```{r bind the approved cancer drugs to kegg_drugs NOT USED}
## select, rename and reorder columns to match kegg_drugs
more_drugs <- unique(dplyr::select(cancer_drugs, "NCIt_drug_id"="NCIt_ID", "drug_name"="drug_name", "target_symbol"="Targets"))

## row bind
# kegg_drugs <- unique(rbind(kegg_drugs, more_drugs))
## do not row bind... 
## some of the drug:target relationships in this dataset are extremely tentative, e.g. pembrolizumab targets MDM2
...
### **Store table licensed drugs and their indications**  
  
```{r create indications table}

indications <- unique(dplyr::select(cancer_drugs, "NCIt_drug_id" = "NCIt_ID", "drug_name"="drug_name", Indications, EMA_approved,FDA_approved, DrugBank_link))

## unnest the indications column
## unnest the Targets column
indications$Indications <- strsplit(x=indications$Indications, split = "; ")
indications <- unnest(data = indications, cols = Indications, keep_empty = TRUE) 
indications <- as.data.frame(indications) ## convert to data frame


```
  
* These data are sourced from the Cancer Drugs Database at https://www.anticancerfund.org/en/cancerdrugs-db  
* These data are saved in the *indications* table:  
  
`r formattable(head(indications))`  
  
```{r write indications table to database}

## create as a table in database
dbWriteTable(conn = con,name = "indications", value = indications, overwrite=TRUE)
...
## delete query to save memory
rm(getInterventionsQ)
```
  

```{r get design group inteventions}
...
## delete query to save memory
rm(local_open_studiesQ)
```

```{r delete studyIDsForSQL to save memory}
rm(studyIDsForSQL)
```
...
```{r delete kwic_genes_populations object to save memory}
rm(kwic_genes_populations)
```
...
```{r delete populations_tokens to save memory}
rm(populations_tokens)
```
...
## delete large tables to save memory
rm(kwic_prior_tx_populations)
rm(text_for_indexing)
```
...
## drop unwanted columns
evidence <- unique(dplyr::select(evidence, variant_id, disease, drugs,  evidence_type, evidence_direction,evidence_level,clinical_significance,evidence_statement, gene_civic_url, variant_civic_url, evidence_civic_url))
...
## add an indicator of what each evidence level means
evidence_levels <- data.frame("evidence_level"=c("A", "B", "C", "D", "E"), 
                              "evidence_level_type"=c("Validated association", "Clinical evidence", "Case study", "Preclinical evidence", "Inferential association"))

## merge with evidence table
evidence <- unique(merge(x=evidence, by.x="evidence_level", y=evidence_levels, by.y="evidence_level"))
```
...
```{r create a table of evidence related to drugs}

drug_evidence <- unique(dplyr::select(evidence, gene, variant, drugs, evidence_type, evidence_direction, evidence_level, evidence_level_type, clinical_significance, evidence_statement, gene_civic_url, variant_civic_url, evidence_civic_url))

## filter and retain only predictive evidence
drug_evidence <- unique(dplyr::filter(drug_evidence, evidence_type %in% c("Predictive")))

```



```{r map drugs from evidence table to NCIt entities}
## The elements of setdiff(x,y) are those elements in x but not in y.
# setdiff(x=tolower(drug_evidence$drugs), y=NCIt_Pharmacologics$Synonyms_lower)

drug_evidence$drugs_lower <- tolower(drug_evidence$drugs)

## some of the "drugs" cited in the evidence table are actually mechanisms (e.g. "aromatase inhibitor")
drug_evidence <- unique(merge(x=drug_evidence, by.x = "drugs_lower", y=unique(dplyr::select(NCIt, "NCIt_ID" = "ID", Synonyms_lower)), by.y="Synonyms_lower"))

## drop unnecessary columns
drug_evidence <- dplyr::select(drug_evidence, -c(drugs_lower, evidence_type))
```


```{r summarise evidence per gene and drug}
## just get the highest level of evidence for each gene-drug combination
drug_evidence_highest <- drug_evidence %>%
    group_by(gene,drugs,clinical_significance) %>%
    filter(evidence_level == min(evidence_level)) %>%
    as.data.frame()


## we need to deal with conflicting evidence


## first, take the highest level of evidence for each gene-variant-drug association and summarise the evidence direction and clinical significance
# drug_evidence_summary <- drug_evidence %>%
#     group_by(gene,variant, drugs, variant_civic_url, NCIt_ID) %>%
#     filter(evidence_level == min(evidence_level)) %>%
#     summarise(
#         evidence_level_type = paste(unique(evidence_level_type), collapse = ", "),
#         evidence_direction_summary = paste(sort(unique(evidence_direction)), collapse = "/"), 
#         clinical_significance_summary = paste(sort(unique(clinical_significance)), collapse = "/")
#     ) %>%
#     as.data.frame()



## add a column that will summarise the rationale
# drug_evidence_summary$rationale <- paste0(drug_evidence_summary$evidence_level_type,
#                                           " linking ",
#                                           drug_evidence_summary$gene, 
#                                           " ", 
#                                           drug_evidence_summary$variant, 
#                                           " with ", 
#                                           drug_evidence_summary$clinical_significance_summary, 
#                                           " to ", 
#                                           drug_evidence_summary$drugs)
...
```{r delete pathwaysInCancer_KGML to save memory}
rm(pathwaysInCancer_KGML)
```
...
```{r delete population_features to save memory}
rm(population_features)
```
