...
```{r index on occurrence gene name}
# ## create a copy
# genetic_eligibilities <- unique(dplyr::select(eligibilities, nct_id, criteria, criterion_type))
# 
# ## split into tokens
# # genetic_eligibilities$tokens <- as.list(corpus::text_tokens(x=genetic_eligibilities$criteria, map_case=FALSE))
# genetic_eligibilities$tokens <- as.list(corpus::text_tokens(x=genetic_eligibilities$criteria, map_case=FALSE, filter= corpus::text_filter(combine = unique(grep(pattern = "-", x=humanGenes$Aliases, value = TRUE)))))
# 
# 
# 
# 
# ## unnest
# genetic_eligibilities <- as.data.frame(unnest(data = genetic_eligibilities, tokens))
# 
# ## join to humanGenes in Aliases so we only retain criteria that mention a human gene synonym
# genetic_eligibilities <- unique(merge(x=genetic_eligibilities, by.x="tokens", y=humanGenes, by.y="Aliases"))
