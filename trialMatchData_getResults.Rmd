...
for(i in 1:length(unique(drugs.targets$drugID))) {
  print(i)
  drugid <- unique(drugs.targets$drugID)[i]
  print(drugid)
  target_id <- getDrugTargetID(drugid)
  (print(target_id))
  drugs.targets$target_id[drugs.targets$drugID==drugid] <- target_id
}
...
```{r get target IDs from KEGG via API}
...
```{r get drug target IDs from KEGG via API}



drugs.targets.preferred$target_id <- NA

tic("get drug target IDs")
for(i in 1:nrow(drugs.targets.preferred)) {
  print(i)
  drugID <- drugs.targets.preferred$drugID[i]
  print(drugID)
  drugs.targets.preferred$target_id[i] <- list(getDrugTargetID(drugID))
  print(drugs.targets.preferred$target_id[i])
}
toc()

## unnest
drugs.targets.preferred <- as.data.frame(unnest(data = drugs.targets.preferred, target_id, keep_empty = TRUE))
```


```{r get drug target symbols from KEGG via API}}
...
drugs.targets.preferred$target_symbol <- NA

tic("get drug target symbols")
for(i in 1:nrow(drugs.targets.preferred)) {
  print(i)
  targetID <- drugs.targets.preferred$target_id[i]
  print(targetID)
  drugs.targets.preferred$target_symbol[i] <- getDrugTargetSymbol(targetID)
  print(drugs.targets.preferred$target_symbol[i])
}
toc()
